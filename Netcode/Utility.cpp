...
void Netcode::Utility::DebugEvent(unsigned int msg, std::initializer_list<unsigned int> filter) {
	...
	for(unsigned int filteredEvt : filter) {
		if(msg == filteredEvt) {
			return;
		}
	}

	std::string possibleEvents = "";

	if(wmTranslation.find(msg) != wmTranslation.end()) {
		auto pairOfIterators = wmTranslation.equal_range(msg);
		
		for(auto i = pairOfIterators.first; i != pairOfIterators.second; ++i) {
			// if its not the first element, print a comma
			if(i != pairOfIterators.first) {
				possibleEvents += ", ";
			}
			possibleEvents += i->second;
		}
	}
