...
FloatType & GetElement(typename std::conditional<std::is_const<FloatType>::value, const uint8_t *, uint8_t *>::type ptr, const InputElement & inputElement) {
...
void Mesh::ApplyTransformation(const Netcode::Float4x4 & mat)
{
	...
	InputElement positionElement;

	for(const InputElement & e : inputLayout) {
	...
	for(LOD & lod : lods) {
		...
		for(uint32_t i = 0; i < lod.vertexCount; ++i) {
			for(const InputElement & e : transformableElements) {
