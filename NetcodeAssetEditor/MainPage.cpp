...
namespace winrt::NetcodeAssetEditor::implementation
{
    ...
    void MainPage::swapChainPanel_Loaded(Windows::Foundation::IInspectable const & sender, Windows::UI::Xaml::RoutedEventArgs const & e)
    {
        ...
        Global::Model = std::make_unique<Netcode::Intermediate::Model>();
    ...
    */
    ...
    fire_and_forget MainPage::AssetCtx_ImportFBX_Click(Windows::Foundation::IInspectable const & sender, Windows::UI::Xaml::RoutedEventArgs const & e)
    {
        ...
        if(file != nullptr) {
            ...
            *Global::Model = Netcode::FBXImporter::FromMemory(buffer.data(), buffer.Length());
            ...
            for(Netcode::Intermediate::Mesh & m : Global::Model->meshes) {
            ...
            for(const Netcode::Intermediate::Mesh & mesh : Global::Model->meshes) {
                auto dcMesh = winrt::make<DC_Mesh>();
                dcMesh.Name(winrt::to_hstring(mesh.name));

                for(const Netcode::Intermediate::LOD & lod : mesh.lods) {
    ...
    winrt::fire_and_forget MainPage::AssetCtx_ImportAnimation_Click(Windows::Foundation::IInspectable const & sender, Windows::UI::Xaml::RoutedEventArgs const & e)
    {
        ...
        if(files != nullptr) {
            ...
            for(uint32_t i = 0; i < files.Size(); ++i) {
                ...
                auto imported = Netcode::FBXImporter::ImportAnimationsFromMemory(buffer.data(), buffer.Length(), Global::Model->skeleton);
                
                auto optAnim = Netcode::FBXImporter::OptimizeAnimation(imported.at(0), Global::Model->skeleton);
    ...
    winrt::fire_and_forget MainPage::FileCtx_SaveManifest_Click(Windows::Foundation::IInspectable const & sender, Windows::UI::Xaml::RoutedEventArgs const & e)
    {
        ...
        if(file != nullptr) {
            ...
            Global::Manifest->materials = Netcode::ConvertToManifestMaterials(Global::Model->materials);
        ...
        if(file != nullptr) {
            ...
            *Global::Model = Netcode::FBXImporter::FromMemory(baseBuffer.data(), baseBuffer.Length());
            ...
            for(const auto & animRef : Global::Manifest->animations) {
                ...
                auto anim = Netcode::FBXImporter::ImportAnimationsFromMemory(animBuffer.data(), animBuffer.Length(), Global::Model->skeleton);
                auto optimizedAnim = Netcode::FBXImporter::OptimizeAnimation(anim.front(), Global::Model->skeleton);
            ...
            if(!Global::Manifest->materials.empty()) {
                Global::Model->materials = Netcode::ConvertToNetcodeMaterials(Global::Manifest->materials);
            }
    ...
    winrt::fire_and_forget MainPage::FileCtx_Compile_Click(Windows::Foundation::IInspectable const & sender, Windows::UI::Xaml::RoutedEventArgs const & e)
    {
        ...
        if(file != nullptr) {
            ...
            std::vector<Netcode::Intermediate::Mesh> copy;
            ...
            auto [rawData, size] = Netcode::AssetExporter::Export(*Global::Model);
