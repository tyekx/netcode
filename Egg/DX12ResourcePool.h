...
namespace Egg::Graphics::DX12 {
	...
	public:
		ResourcePool() : defaultPermanentHeaps { D3D12_HEAP_TYPE_DEFAULT },
		...
		uint64_t CreateStructuredBuffer(ResourceType type, UINT64 sizeInBytes, UINT strideInBytes, ResourceState initialState, ResourceFlags optFlags = ResourceFlags::NONE) {
			ResourceDesc rDesc;
			rDesc.type = type;
		...
		uint64_t CreateTypedBuffer(ResourceType type, UINT64 sizeInBytes, DXGI_FORMAT format, ResourceState initialState, ResourceFlags optFlags = ResourceFlags::NONE) {
			ResourceDesc rDesc;
			rDesc.type = type;
			rDesc.dimension = ResourceDimension::BUFFER;
			rDesc.height = 1;
