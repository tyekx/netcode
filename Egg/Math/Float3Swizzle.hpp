...
namespace Egg {
    namespace Math {
        ...
        template<typename F, typename I, typename B, int nBase, int s0 = 0, int s1 = 0, int s2 = 0>
        class Float3Swizzle {
            float v[nBase];
        public:
        ...
        F Clamp(const F & low, const F & high) const noexcept {
            F t = *this;
            return F { (t.x < low.x) ? low.x: ((t.x > high.x) ? high.x :t.x), (t.y < low.y) ? low.y: ((t.y > high.y) ? high.y :t.y), (t.z < low.z) ? low.z: ((t.z > high.z) ? high.z :t.z) };
        }
        ...
        F Sign() const noexcept {
            F t = *this;
            return F { (t.x > 0.0f) ? 1.0f : ((t.x < 0.0f) ? -1.0f : 0.0f), (t.y > 0.0f) ? 1.0f : ((t.y < 0.0f) ? -1.0f : 0.0f), (t.z > 0.0f) ? 1.0f : ((t.z < 0.0f) ? -1.0f : 0.0f) };
        }

        I Round() const noexcept {
            F t = *this;
            return I { (int)(t.x + 0.5f), (int)(t.y + 0.5f), (int)(t.z + 0.5f) }; 
        }

        F Saturate() const noexcept {
            F t = *this;
            return t.Clamp(F { 0, 0, 0 }, F { 1, 1, 1 });
        }
        ...
        F Normalize() const noexcept {
            F t = *this;
            float len = t.Length();
             return F { t.x / len , t.y / len , t.z / len  };
        }

        B IsNan() const noexcept {
            F t = *this;
            return B { std::isnan(t.x), std::isnan(t.y), std::isnan(t.z) };
        }

        B IsFinite() const noexcept {
            F t = *this;
            return B { std::isfinite(t.x), std::isfinite(t.y), std::isfinite(t.z) };
        }
        ...
        F operator%(const F & rhs) const noexcept {
            F t = *this;
            return F { ::fmodf(t.x, rhs.x), ::fmodf(t.y, rhs.y), ::fmodf(t.z, rhs.z) };
        }

        I Ceil() const noexcept {
            F t = *this;
            return I { (int)::ceil(t.x), (int)::ceil(t.y), (int)::ceil(t.z) };
        }

        I Floor() const noexcept {
            F t = *this;
            return I { (int)::floor(t.x), (int)::floor(t.y), (int)::floor(t.z) };
        }

        F Exp2() const noexcept {
            F t = *this;
            return F { ::pow(2.0f,t.x), ::pow(2.0f,t.y), ::pow(2.0f,t.z) };
        }

        I Trunc() const noexcept {
            F t = *this;
            return I { (int)t.x, (int)t.y, (int)t.z };
        }
        ...
        F operator%(float v) const noexcept {
            F t = *this;
            return F { ::fmodf(t.x, v), ::fmodf(t.y, v), ::fmodf(t.z, v) };
        }

        F Cross(const F & rhs) const noexcept {
            F t = *this;
            return F { t.y * rhs.z - t.z * rhs.y, t.z * rhs.x - t.x * rhs.z, t.x * rhs.y - t.y * rhs.x };
        }
